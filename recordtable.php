...
<head>
    <title>Bảng giá trị cảm biến</title>
...
<body>
    <div class="topnav">
        <h3>Bảng giá trị cảm biến</h3>
    </div>

    <br>

    <h3 style="color: #0c6980;">Bảng dữ liệu</h3>
    ...
    <table class="styled-table" id="table_id">
        ...
        <tbody id="tbody_table_record">
            ...
            $sql = 'SELECT * FROM esp32_dht11_leds_record ORDER BY date, time';

            // foreach ($pdo->query($sql) as $row) {
            //     $date = date_create($row['date']);
            //     $dateFormat = date_format($date, "d-m-Y");
            //     $time = date_create($row['time']);
            //     $timeFormat = date_format($time, "H:i:s");
            foreach ($pdo->query($sql) as $row) {
                $dateFormat = isset($row['date']) ? date_format(date_create($row['date']), "d-m-Y") : "N/A";
                $timeFormat = isset($row['time']) ? date_format(date_create($row['time']), "H:i:s") : "N/A";
                $num++;
                echo '<tr>';
                echo '<td>' . $num . '</td>';
    ...
    <div class="btn-group">
        <button class="button" id="btn_prev" onclick="prevPage()">Trở về</button>
        <button class="button" id="btn_next" onclick="nextPage()">Tiếp tục</button>
        <button class="button" id="btn_update" onclick="updateData()">UPDATE DATA</button>

        <div style="display: inline-block; position:relative; border: 0px solid #e3e3e3; float: center; margin-left: 2px;;">
            <p style="position:relative; font-size: 14px;"> Bảng : <span id="page"></span></p>
        </div>
        ...
        <button class="button" id="btn_apply" onclick="apply_Number_of_Rows()">Chấp nhận</button>
    </div>

    <br>

    <script>
        //------------------------------------------------------------Show 1st page of table
        //var current_page = 1;
        //var records_per_page = 10;
        //var l = document.getElementById("table_id").rows.length---------------------------------------------
        var records_per_page = 10;
        var l = document.getElementById("table_id").rows.length;
        var current_page = Math.ceil(l / records_per_page); // Tính trang cuối cùng
        ...
        // Gọi hàm để hiển thị trang cuối cùng
